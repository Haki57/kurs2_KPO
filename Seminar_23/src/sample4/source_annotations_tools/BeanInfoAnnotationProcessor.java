...
//TODO: pay attention at the annotations below (otherwise may not work!):
@SupportedAnnotationTypes("sample4.source_annotations_tools.PropertyForEditor")
@SupportedSourceVersion(SourceVersion.RELEASE_11)
public class BeanInfoAnnotationProcessor extends AbstractProcessor {
    @Override
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (TypeElement t : annotations) {
            Map<String, PropertyForEditor> props = new LinkedHashMap<>();
            ...
            for (Element e : roundEnv.getElementsAnnotatedWith(t)) {
                ...
                for (int i = 0; !found && i < prefixes.length; i++)
                    ...
                    props.put(name, e.getAnnotation(PropertyForEditor.class));
                    }
                if (!found) processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
                        "@PropertyForEditor must be applied to getXxx, setXxx, or isXxx method", e);
     ...
     */
    private void writeBeanInfoFile(String beanClassName, Map<String, PropertyForEditor> props)
    throws IOException {
        JavaFileObject sourceFile = processingEnv.getFiler().createSourceFile(
                beanClassName + "BeanInfo");
System.out.println("writing source file for : " + beanClassName + "BeanInfo");
System.out.println("source file = " + sourceFile);
        ...
        out.println(" try");
        out.println(" {");
        for (Map.Entry<String, PropertyForEditor> e : props.entrySet()) {
